...
void OgreConsole::print(const Ogre::String &text)
{
   ...
   int start=0,count=0, lastwordbreak=-1;
   int len=text.length();
   for(int c=0;c<len;c++){
      if(str[c]=='\n'){
         lines.push_back( text.substr(start, (c-start+1) ) );
         start = c;
      } else if( (c-start+1) >= CONSOLE_LINE_LENGTH ) {
         lines.push_back( text.substr(start, (lastwordbreak-start+1) ) );
         start = lastwordbreak+1;
      } else {
         if( str[c] == ' ' || str[c] == '-' || str[c] == ';' || str[c] == ':' ) lastwordbreak = c;
      }
   }
   if( start < CONSOLE_LINE_LENGTH )
